import { QuizIndex, MateriaData, Quiz } from '@/types/quiz';
...
// Genera quiz per simulazione seguendo la struttura dell'esame
export async function generaQuizSimulazione(): Promise<Array<Quiz & { materia: string }>> {
  const index = await getQuizIndex();
  ...
  for (const materia of index.materie) {
    ...
    // Prendi il numero di domande richieste per l'esame
    const quizSelezionati = quizMateria.slice(0, materia.domande_esame);

    // Aggiungi la materia a ogni quiz
    quizSelezionati.forEach(q => {
