...
import { Quiz, SimulazioneRisposta, StatisticheMateria } from '@/types/quiz';
...
export const useQuizStore = create<QuizStore>()(
  persist(
    (set, get) => ({
      ...
      quizSimulazione: [],
      risposteSimulazione: [],
      tempoRimanente: 60 * 60, // 60 minuti in secondi
      ...
      prossimoQuiz: () => {
        // Questa azione verrà chiamata dalla pagina che gestisce la lista quiz
        set({
          rispostaSelezionata: null,
          mostraSoluzione: false,
          tempoInizio: Date.now(),
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          // Converti array in Set dopo rehydration
